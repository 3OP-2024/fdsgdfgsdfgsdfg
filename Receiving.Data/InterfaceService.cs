...
{ 
   ...
   public interface IVInventoryRepository : IRepository<V_HR_PR_EquipmentInventory>
    ...
    {
        string GetRunning();
        HelperPaging<HR_PR_EquipmentReceivingHeader> GetDataList(Search search);
    }
    public class ReceivingHeaderRepository : Repository<HR_PR_EquipmentReceivingHeader>, IReceivingHeaderRepository
    {
        public ReceivingHeaderRepository(Context repositoryContext)
            : base(repositoryContext)
        {
        }
        public HelperPaging<HR_PR_EquipmentReceivingHeader> GetDataList(Search search)
        {
            try
            {
                var items = RepositoryContext.HR_PR_EquipmentReceivingHeader
                                                                    .Include(s => s.HR_PR_EquipmentReceivingDetail)
                                                                    .Include(s => s.V_HR_MT_Department)
                                                                    .Where(l =>
                                                                         (l.RunningID.Contains(search.RunningID) ||  string.IsNullOrEmpty(search.RunningID))
                                                                        && (l.RequestNo.Contains(search.RequestNo) ||  string.IsNullOrEmpty(search.RequestNo))
                                                                        && (l.ReceivingType.Contains(search.TypeID) ||  string.IsNullOrEmpty(search.TypeID))
                                                                        && (l.VendorName.Contains(search.critiriaVendor) || l.VendorID.Contains(search.critiriaVendor) ||  string.IsNullOrEmpty(search.critiriaVendor))
                ...
                var result = new HelperPaging<HR_PR_EquipmentReceivingHeader>(items.OrderByDescending(l => l.RunningID), (search.page != 0 ? search.page : 1), search.PageSetNumber, search.PageOn);
        ...
        public string GetRunning()
        {
            ...
            try
            {
                ...
                var result = RepositoryContext.HR_PR_EquipmentReceivingHeader
            ...
            credential = $"{DateTime.Now.Year.ToString("0000")}{DateTime.Now.Month.ToString("00")}{DateTime.Now.Day.ToString("00")}{runningId.ToString("00")}";
    ...
    public interface IReceivingDetailRepository : IRepository<HR_PR_EquipmentReceivingDetail>
    {
    }
    public class ReceivingDetailRepository : Repository<HR_PR_EquipmentReceivingDetail>, IReceivingDetailRepository
    {
        public ReceivingDetailRepository(Context repositoryContext)
            : base(repositoryContext)
        {
        }
    }
